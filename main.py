class Graph:
    ...
    def dfs2(self, curr_node, cuv, Traseu, lista_nr_noduri, Traseu_Moore, str_finala):
        ...
        while len(cuv) > 0:
            ...
            for next_node, simbol in self.edges.get(curr_node, []):
                i += 1
                if simbol == cuv[0]:
                    Traseu.append(next_node)
                    Traseu_Moore.append(lista_nr_noduri[next_node])
                    cuv = cuv[1:]
                    if len(cuv) == 0 and next_node in str_finala:
                        print("DA")
                        print("".join(Traseu_Moore))
                        print("Traseu: ", *Traseu)
                        break
                    if len(cuv) == 0 and next_node not in  str_finala:
                        print("NU")
                        break
                    self.dfs2(next_node, cuv, Traseu, lista_nr_noduri, Traseu_Moore, str_finala)
                    break
...
for linie in f:
    ...
    Traseu = []
    Traseu_Moore = []
    Traseu.append(int(stare_initiala))
    Traseu_Moore.append(lista_nr_noduri[int(stare_initiala)])
    g.dfs2(0, cuv, Traseu, lista_nr_noduri, Traseu_Moore, stari_finale)